  ...
  const renderDetails = () => (
    <form onSubmit={handleNext} className="bg-white rounded-sm p-8 md:p-10 shadow-2xl text-stone-900 animate-in slide-in-from-right-8 duration-300 relative">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <div className="group">
          <label htmlFor="name" className="block text-xs font-bold uppercase tracking-wide text-stone-500 mb-2 group-focus-within:text-brand-600 transition-colors">Full Name</label>
          <div className="relative">
            <User className="absolute left-3 top-3.5 h-5 w-5 text-stone-400 group-focus-within:text-brand-500 transition-colors" aria-hidden="true" />
            <input 
              id="name"
              required
              type="text" 
              className="w-full pl-10 pr-4 py-3 bg-stone-50 border border-stone-200 focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all rounded-sm font-medium"
        ...
        <div className="group">
          <label htmlFor="email" className="block text-xs font-bold uppercase tracking-wide text-stone-500 mb-2 group-focus-within:text-brand-600 transition-colors">Email Address</label>
          <div className="relative">
            <Mail className="absolute left-3 top-3.5 h-5 w-5 text-stone-400 group-focus-within:text-brand-500 transition-colors" aria-hidden="true" />
            <input 
              id="email"
              required
              type="email" 
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <div className="group">
          <label htmlFor="phone" className="block text-xs font-bold uppercase tracking-wide text-stone-500 mb-2 group-focus-within:text-brand-600 transition-colors">Phone Number</label>
          <div className="relative">
            <Phone className="absolute left-3 top-3.5 h-5 w-5 text-stone-400 group-focus-within:text-brand-500 transition-colors" aria-hidden="true" />
            <input 
              id="phone"
              required
              type="tel" 
        ...
        <div className="group">
          <label htmlFor="course" className="block text-xs font-bold uppercase tracking-wide text-stone-500 mb-2 group-focus-within:text-brand-600 transition-colors">Course of Interest</label>
          <div className="relative">
            <select 
              id="course"
              required
              className="w-full px-4 py-3 bg-stone-50 border border-stone-200 focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all appearance-none rounded-sm font-medium cursor-pointer"
            ...
            <ChevronRight className="absolute right-4 top-4 h-4 w-4 text-stone-400 rotate-90 pointer-events-none" />
          </div>
        </div>
      </div>
      ...
      <button 
        type="submit"
        className="w-full bg-stone-900 text-white font-bold text-lg py-4 hover:bg-brand-600 transition-all shadow-lg flex items-center justify-center gap-2 rounded-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 mt-4"
      >
    ...
    <div className="bg-white rounded-sm p-8 md:p-10 shadow-2xl text-stone-900 animate-in slide-in-from-right-8 duration-300 relative">
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">Choose a time to visit the academy.</p>
        </div>
      </div>

      {/* Date Scroller */}
      <div className="mb-8" role="group" aria-label="Select interview date">
        <label className="block text-xs font-bold uppercase tracking-wide text-stone-500 mb-3" id="date-label">Select Date</label>
        <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide -mx-2 px-2 py-1">
          {dates.map((date, idx) => (
            <button
              key={idx}
              type="button"
              ...
              className={`flex-shrink-0 w-20 h-24 rounded-lg border-2 flex flex-col items-center justify-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500 ${
                selectedDate?.toDateString() === date.toDateString()
                  ? 'border-brand-500 bg-brand-50 text-brand-700 shadow-md transform -translate-y-1'
                  : 'border-stone-100 bg-white text-stone-400 hover:border-brand-200 hover:text-stone-600'
              }`}
            >
              <span className="text-xs uppercase font-bold" aria-hidden="true">{date.toLocaleDateString('en-US', { weekday: 'short' })}</span>
              <span className="text-2xl font-bold font-serif my-1" aria-hidden="true">{date.getDate()}</span>
              <span className="text-[10px] uppercase" aria-hidden="true">{date.toLocaleDateString('en-US', { month: 'short' })}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Time Grid */}
      <div className={`mb-8 transition-opacity duration-300 ${!selectedDate ? 'opacity-50 pointer-events-none' : 'opacity-100'}`} role="group" aria-label="Select interview time">
        <label className="block text-xs font-bold uppercase tracking-wide text-stone-500 mb-3" id="time-label">Available Times</label>
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
          {['09:00 AM', '10:00 AM', '11:00 AM', '01:00 PM', '02:00 PM', '03:00 PM', '04:00 PM'].map((time) => (
            <button
              ...
              className={`py-2.5 px-1 text-sm font-medium rounded-md border transition-all focus:outline-none focus:ring-2 focus:ring-brand-500 ${
                selectedTime === time
      ...
      <div className="flex gap-4">
        <button 
          type="button"
          onClick={handleBack}
          className="px-6 py-4 bg-stone-100 text-stone-600 font-bold rounded-sm hover:bg-stone-200 transition-colors focus:outline-none focus:ring-2 focus:ring-stone-400"
        >
        ...
        <button 
          ...
          className="flex-1 bg-stone-900 text-white font-bold text-lg py-4 hover:bg-brand-600 disabled:bg-stone-200 disabled:text-stone-400 disabled:cursor-not-allowed transition-all shadow-lg flex items-center justify-center gap-2 rounded-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2"
        >
          Review Application
          <ChevronRight className="h-5 w-5" aria-hidden="true" />
        </button>
    ...
    <div className="bg-white rounded-sm p-8 md:p-10 shadow-2xl text-stone-900 animate-in slide-in-from-right-8 duration-300 relative">
      ...
      <div className="flex gap-4">
        ...
        <button 
          ...
          className="flex-1 bg-brand-600 text-white font-bold text-lg py-4 hover:bg-brand-700 transition-all shadow-lg flex items-center justify-center gap-2 rounded-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2"
        >
          Confirm Booking
